...
/**
 * 通过 DOCX → soffice --headless 转 PDF（高保真）
 * 若无 LibreOffice，抛错提示安装
 */
export async function exportDocxToPdf(person: ArchivePerson, templatePath: string, outputPath: string): Promise<void> {
  const data = prepareDocxData(person)
  const tmpDir = mkdtempSync(join(tmpdir(), 'pdf-'))
  const docxPath = join(tmpDir, 'temp.docx')

  try {
    renderDocx(data, templatePath, docxPath)

    // 查找 soffice 路径
    const sofficePaths = process.platform === 'win32'
      ? ['soffice.exe', 'C:\\Program Files\\LibreOffice\\program\\soffice.exe', 'C:\\Program Files (x86)\\LibreOffice\\program\\soffice.exe']
      : ['soffice', 'libreoffice']

    let soffice: string | null = null
    for (const p of sofficePaths) {
      if (existsSync(p) || await isExecutable(p)) {
        soffice = p
        break
      }
    }
    ...
    if (!soffice) {
      throw new Error('未找到 LibreOffice。请安装 LibreOffice 后重试：https://www.libreoffice.org/download/')
    }

    await execFileP(soffice, ['--headless', '--convert-to', 'pdf', '--outdir', tmpDir, docxPath])
    const pdfPath = join(tmpDir, 'temp.pdf')

    if (!existsSync(pdfPath)) {
      throw new Error('PDF 转换失败：LibreOffice 未生成输出文件')
    }

    const pdfBuf = readFileSync(pdfPath)
    writeFileSync(outputPath, pdfBuf)
  } finally {
...
async function isExecutable(p: string): Promise<boolean> {
  ...
  // 检查 PATH
  try {
    await execFileP(process.platform === 'win32' ? 'where' : 'which', [p])
    return true
  } catch {
    return false
  }
